/**
 * Database Indexer - Main Orchestrator
 *
...
const EXTRACTORS: Record<ExtractorName, () => Promise<any>> = {
...
async function logBatchRun(
  ...
  const entitiesCreated = result.entities.created;
  const entitiesUpdated = result.entities.updated;
  const interactionsCreated =
    result.calls.created + result.emails.created + result.telegram.created;

  await query(
    `INSERT INTO batch_runs (id, started_at, ended_at, type, status, entities_created, entities_updated, interactions_created, error, data)
     VALUES ($1, $2, NOW(), 'full', 'completed', $3, $4, $5, $6, $7)`,
    [
      runId,
      ...
      JSON.stringify({
        ...
        aliases: result.entities.aliases,
      }),
    ]
  );
}
...
async function runIndexer(options: IndexerOptions = {}): Promise<void> {
  ...
  for (const name of toRun) {
    try {
      ...
      const extractResult = await extractor();
  ...
  // Log batch run
  try {
    await logBatchRun(startTime, result);
  } catch (error: any) {
