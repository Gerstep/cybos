...
└─────────────────────────────────────────────────────────┘
        ...
        ▼                   ▼                   ▼
┌───────────────┐   ┌───────────────┐   ┌───────────────┐
│  MCP SERVERS  │   │  PostgreSQL   │   │    OUTPUTS    │
│ Perplexity    │   │  (pgvector)   │   │ /deals/       │
...
- **Database**: Call interactions indexed in PostgreSQL via `/cyber-reindex`
...
- **Database**: Email interactions indexed in PostgreSQL via `/cyber-reindex`
...
**Data flow:**
```
PostgreSQL (context graph) → getExplorerDashboard() → /api/explorer → ExplorerPage.tsx
```
...
Cybos queries Google Calendar for meeting schedules.
...
| Calendar | `mcp__calendar__list-events` with timeMin/timeMax | Event list |
...
- **gmail**: Email management for VC deal flow ✅
- **calendar**: Google Calendar for meeting schedules ✅
...
| **gmail** | Email management for VC deal flow ✅ | `search_emails`, `read_email`, `draft_email`, `send_email`, `download_attachment`, `modify_email`, `list_email_labels` |
| **calendar** | Google Calendar for meeting schedules ✅ | `list_events`, `get_event`, `create_event`, `update_event`, `delete_event` |
...
mcp__typefully__typefully_create_draft ✅ Validated
  - social_set_id: number (default: 161806)
  - requestBody: {
...
Default account: stepa — cyber/acc (ID: 161806, @sgershuni)
Connected platforms: Twitter (@cyntro_py), LinkedIn (@sgershuni)
...
mcp__gmail__read_email
  - messageId: string
  - Returns: Full email content (subject, from, to, date, body, thread ID)

mcp__gmail__draft_email (RECOMMENDED - no confirmation required)
  - to: string[]
  - subject: string
  - body: string
  - cc: string[] (optional)
  ...
  - Returns: { draftId, message }

mcp__gmail__send_email (CONFIRMATION REQUIRED)
  - to: string[]
  - subject: string
  - body: string
  - cc: string[] (optional)
...
mcp__gmail__download_attachment
  - messageId: string
  - attachmentId: string
  - savePath: string (optional, defaults to current directory)
  - filename: string (optional)
  - Returns: { path: saved_path, size: bytes }
...
mcp__gmail__list_email_labels
  - Returns: All available Gmail labels (inbox, sent, custom labels)

mcp__gmail__get_or_create_label
  - name: string
  - Returns: { id, name } (creates if doesn't exist)
...
Configuration:
- Server: @gongrzhe/server-gmail-autoauth-mcp (NPX-based, stdio compatible)
- Auth: OAuth 2.0 with auto-authentication flow
- Scopes: gmail.modify, gmail.settings.basic
- Credentials: ~/.gmail-mcp/gcp-oauth.keys.json
- Token: ~/.gmail-mcp/credentials.json (auto-generated after auth)

Integration with /deals/:
- Auto-detects company from sender domain
- Loads context from /deals/<company>/.cybos/context.md
- Saves attachments to /deals/<company>/materials/ or /deals/<company>/updates/
- Creates new deal folders when prompted

Safety features:
- ALWAYS asks for confirmation before sending emails
- ALWAYS asks before creating deal folders
- Draft-first workflow (draft_email instead of send_email)
- "TO ANSWER" label for follow-up tracking

Testing status: ✅ Fully operational
- Tested: search_emails, read_email
- See docs/GMAIL-TESTING.md for setup instructions
```
...
5. CREATE DRAFT
   ...
   └─ Social set: 161806 (stepa @sgershuni)
...
The database indexer provides a unified PostgreSQL-based context graph for all entities, interactions, and extracted items.
...
entity_aliases     -- Name variations for fuzzy matching
...
files              -- File registry for sync tracking (checksums)
batch_runs         -- Indexer run logs with cost tracking

-- Key features
- Full-text search via tsvector + GIN indexes
- Fuzzy name matching via pg_trgm extension
- Temporal queries via timestamps
- JSONB for flexible participant storage
```
...
├── client.ts               # Shared DB connection pool
├── init.ts                 # Create database and tables
├── index.ts                # Main batch indexer
├── migrate.ts              # Migration from .index.json
├── extract-llm.ts          # Claude Haiku extraction runner
├── entity-resolver.ts      # Entity dedup/matching/creation
...
Multi-stage matching for aggressive deduplication:
...
| 4 | Alias exact match | 0.95 | Return existing entity |
| 5 | Fuzzy name match (>0.5) | 0.7-0.9 | Return existing, add alias |
| 6 | No match found | - | Create new entity (or candidate) |
...
```bash
# Start PostgreSQL
cd scripts/db && docker compose up -d
...
└── scripts/                    # Utility scripts
    ...
    └── db/                     # PostgreSQL database indexer
        ├── docker-compose.yml  # PostgreSQL 16 + pgvector
        ...
        ├── entity-resolver.ts  # Entity dedup/matching
        ├── query.ts            # Query interface + CLI
